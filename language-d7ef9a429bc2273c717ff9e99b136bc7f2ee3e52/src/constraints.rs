...
impl ConstraintSolver {
    ...
    fn solve_hinge_constraint(
        ...
        let perp1 = if hinge_axis.cross(Vec3::new(1.0, 0.0, 0.0)).magnitude() > 0.1 {
            ...
            hinge_axis.cross(Vec3::new(0.0, 1.0, 0.0)).normalized()
        };
        let perp2 = hinge_axis.cross(perp1).normalized();
    ...
    fn solve_angular_constraint(
        ...
        let relative_rotation = orientation_b * orientation_a.inverse();
        let axis_angle = relative_rotation.to_axis_angle();
        let current_angle = axis_angle.1;

        // Calculate constraint violation
        let constraint_violation = current_angle - rest_angle;

        // Use dt2 for stability and damping calculations
        let compliance = 1.0 / (stiffness * dt2 * dt2);
        let stabilization_factor = dt2 * 0.1; // Use dt2 for stabilization

        // Calculate constraint force
        let effective_mass = 1.0 / (inertia_a + inertia_b + compliance);
        let delta_lambda = -constraint_violation * effective_mass - stabilization_factor * (*lambda);
