...
impl Interpreter {
    ...
    fn pattern_matches(&self, pattern: &Pattern, value: &Value) -> RuntimeResult<bool> {
        match (pattern, value) {
            ...
            (Pattern::Literal(lit_expr), _) => {
                // Would need to evaluate literal and compare
                Ok(true) // Simplified
            }
            (Pattern::Variable(_), _) => Ok(true), // Variables always match
            _ => Ok(false),
        }
    }
            ...
            Ok(value) => {
                ...
                match value {
                    Value::Struct { fields, .. } => {
                        if let Some(field_value) = fields.get(field) {
                            Ok(field_value.clone())
                        } else if let Some(fb_expr) = fallback {
                            self.eval_expression(fb_expr)
                        } else {
                            Ok(Value::Unit)
                        }
                    }
                    _ => {
                        if let Some(fb_expr) = fallback {
                            self.eval_expression(fb_expr)
                        } else {
                            Ok(Value::Unit)
                        }
    ...
    #[cfg(feature = "jit")]
    pub fn jit_compile_function(&mut self, func: &FunctionDeclaration) -> Result<String, JitError> {
    ...
    #[cfg(not(feature = "jit"))]
    pub fn jit_compile_function(&mut self, _func: &FunctionDeclaration) -> Result<String, String> {
        Err("JIT compilation not enabled".to_string())
    }

    /// Check if a function can be JIT compiled
    #[cfg(feature = "jit")]
    pub fn can_jit_compile(&self, func: &FunctionDeclaration) -> bool {
        if let Some(ref jit) = self.jit_context {
    ...
    #[cfg(not(feature = "jit"))]
    pub fn can_jit_compile(&self, _func: &FunctionDeclaration) -> bool {
        false
    }
